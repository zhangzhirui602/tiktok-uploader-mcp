...
> **时区格式：** `--timezone` 必须使用 `洲/城市` 格式，例如 `Europe/Stockholm`、`Asia/Shanghai`、`America/New_York`。不能只写城市名（如 `Stockholm` 会报错）。

> **批量上传限制：** 批量上传只支持**等间隔**定时，无法为每个视频单独指定不同时间。如需自定义每个视频的发布时间，请使用 Python API。

> **失败行为：** 如果定时发布设置失败，该视频上传会直接中止，**不会**以"立即发布"兜底。
...
> **重要：** 请使用 TikTok Sounds 面板中显示的**准确歌名和歌手名**，歌名拼写必须完全正确（不区分大小写），否则会 fallback 到搜索结果第一条。

> **局限性：** TikTok 每次搜索只加载约 10–20 条结果。如果目标歌曲排在这批结果之外，则不会出现在列表中，上传工具会自动 fallback 到第一条。建议使用知名度较高的歌曲，或同时提供歌手名以帮助 TikTok 更准确地返回目标歌曲。

> **批量上传注意：** `--sound-name` 只传一次时所有视频使用同一首歌；如需每个视频不同音乐，必须重复传入且数量与 `-v` 完全一致，否则多余的视频不会添加音乐。
